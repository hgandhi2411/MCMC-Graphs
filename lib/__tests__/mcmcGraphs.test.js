...
describe('graphmc edge lengths', () => {
  it('should correctly calculate distances', () => {
    const graph = new mcmcGraphs.Graph(3);
    graph.connect(0, 1);
    graph.nodeCoords(0, 0, 0);
    graph.nodeCoords(1, 3, 4);
    const result = mcmcGraphs.distance(graph);
    const expected = 5.0;
    assert(result[0] === expected, 'the distances are not correct');
  });
});
